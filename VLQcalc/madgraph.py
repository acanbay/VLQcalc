...
"""
class MG5:
    def __init__(self,VLQ,model="VLQ_v4"):
        self.VLQ = VLQ

        # Checking the model
        if model.upper() not in ["VLQ_UFO", "VLQ_V4", "VLQ_V5"]:
        ...
        if model.upper() == "VLQ_UFO":
            ...
            self.model = "VLQ_v5"
            self.text = f"import model {self.model}_{VLQ.FNS}FNS_NLO_UFO-3rd"
            if VLQ.LR:
                self.text += "\n"
            else:
                self.text += "L\n"
        ...
        self.Identication = ""
        self.Process = []
        self.shower = "OFF" #pythia8
        self.detector = "OFF" #ExRoot, MadAnalysis
        self.analysis = "OFF"
        self.madspin = "OFF"
        self.reweight = "OFF"
        self.runInputs = []
